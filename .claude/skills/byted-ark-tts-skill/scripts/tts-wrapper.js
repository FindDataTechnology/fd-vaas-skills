...
/**
 * byted-ark-tts-skill — CLI wrapper.
 *
...
async function handleCreate(args) {
  ...
  const result = await tts.synthesizeSpeech(params, options);
  ...
  console.log(`⏰ 预计时长: ${result.estimated_duration_seconds} 秒`);
  console.log(`\n💾 本地文件路径:`);
  console.log(`   ${result.local_path}`);
  if (result.captions_path) {
    console.log(`📝 字幕文件路径 (${result.captions_count} 个字/词，官方逐字时间戳):`);
