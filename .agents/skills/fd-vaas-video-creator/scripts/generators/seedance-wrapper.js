...
/**
 * fd-vaas-video-creator - Seedance AI 视频生成 v4
 *
 * 🎯 核心职责变更：
...
function resolveBridgePath() {
  ...
  return path.join(__dirname, '..', '..', '..', 'scripts', 'litellm-bridge.py');
}
...
function loadEnvFromProject() {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (fs.existsSync(envPath)) {
      ...
      if (out.VOLC_AGENT_API_KEY || out.VIDEO_PROVIDER) return out;
    }
    const parent = path.dirname(dir);
    if (parent === dir) break;
    dir = parent;
...
async function handleCreateViaLiteLLM(args) {
  const bridgePath = resolveBridgePath();
  const env = loadEnvFromProject();

  const prompt = getArgValue(args, '--prompt') || '';
  const duration = getArgValue(args, '--duration') || '5';
  const ratio = getArgValue(args, '--ratio') || 'adaptive';
  const resolution = getArgValue(args, '--resolution') || '720p';
  const model = getArgValue(args, '--model') || env.VIDEO_MODEL || 'doubao-seedance-2.0';
  const apiKey = getArgValue(args, '--api-key') || env.VOLC_AGENT_API_KEY || env.ARK_API_KEY || '';
  const baseUrl = getArgValue(args, '--base-url') || env.SEEDANCE_BASE_URL || '';
  const isAsync = hasArg(args, '--async') || hasArg(args, '--async-mode');
  ...
  // Output path — use the same download dir convention
  const ts = makeTimestampLiteLLM();
  const taskDir = path.join(DEFAULT_DOWNLOAD_DIR, `litellm-${ts}`);
  fs.mkdirSync(taskDir, { recursive: true });
  const outPath = path.join(taskDir, '01-content.mp4');
  ...
  if (isAsync) bridgeArgs.push('--async');
  if (apiKey) bridgeArgs.push('--api-key', apiKey);
  if (baseUrl) bridgeArgs.push('--base-url', baseUrl);

  const result = spawnSync('python3', bridgeArgs, {
    encoding: 'utf8',
    maxBuffer: 10 * 1024 * 1024,
    timeout: (isAsync ? 60 : 600) * 1000,
    env: { ...process.env, ...env },
  });

  if (result.error) {
    console.log(JSON.stringify({
      ok: false, error_code: 'BRIDGE_ERROR',
  ...
  if (result.status !== 0) {
    console.log(JSON.stringify({
      ok: false, error_code: 'BRIDGE_EXIT',
  ...
  try { data = JSON.parse(result.stdout.trim()); }
    ...
    console.log(JSON.stringify({ ok: false, error_code: 'PARSE_ERROR', error_message: result.stdout.slice(0, 200) }));
    process.exit(1);
  }

  // Output in the same format as the native seedance flow
  if (data.status === 'submitted' || isAsync) {
    console.log('');
    console.log('='.repeat(50));
    console.log('✅ 视频生成任务已提交！');
    console.log('');
    console.log(`🆔 任务 ID: ${data.task_id || 'litellm-task'}`);
    console.log(`🤖 使用模型: ${data.model || model}`);
    if (data.provider) console.log(`🏢 Provider: ${data.provider}`);
    console.log('');
    console.log('💡 LiteLLM bridge 模式：任务已提交');
    console.log('='.repeat(50));
  } else if (data.local_path) {
    console.log('');
    console.log('🎉 视频生成完成！');
    console.log('');
    console.log(`🎬 任务: ${data.task_id || 'litellm-task'}`);
    console.log(`💡 提示词: ${prompt.slice(0, 50)}${prompt.length > 50 ? '...' : ''}`);
    console.log(`🤖 模型: ${data.model || model}`);
    if (data.provider) console.log(`🏢 Provider: ${data.provider}`);
    if (data.download_url) console.log(`🔗 在线视频地址: ${data.download_url}`);
    console.log(`💾 已自动下载到本地: ${data.local_path}`);
    console.log('');
  } else {
...
async function handleCreate(args) {
  ...
  // 0.5️⃣ LiteLLM bridge 模式：短路径，直接调用 Python bridge
  // 触发条件：--litellm flag 或 VIDEO_PROVIDER != volcengine
  const useLiteLLM = hasArg(args, '--litellm');
  const env = loadEnvFromProject();
  const videoProvider = (env.VIDEO_PROVIDER || 'volcengine').toLowerCase();
  if (useLiteLLM || videoProvider !== 'volcengine') {
    await handleCreateViaLiteLLM(args);
    return;
  }
