...
/**
 * fd-vaas-video-creator - Seedream wrapper.
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a.startsWith('--')) {
      const key = a.slice(2);
      const next = argv[i + 1];
      const BOOL_KEYS = new Set(['watermark', 'enable-web-search', 'sequential', 'save-api-key', 'no-preference', 'litellm']);
...
async function cmdCreate(args) {
  ...
  try {
    ...
    // Auto-route: --litellm flag or IMAGE_PROVIDER != volcengine
    const useBridge = args.litellm || core.shouldUseLiteLLM(options);
    const result = useBridge
      ? await core.generateImageViaLiteLLM(params, options)
      : await core.generateImage(params, options);
...
create 选项:
  ...
  --litellm                 强制走 LiteLLM bridge（多 provider，由 .env 的 IMAGE_PROVIDER/IMAGE_MODEL 决定）
