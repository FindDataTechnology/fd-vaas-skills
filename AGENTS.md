...
2. **Distribution** - the two publish skills ship the asset to platforms. `fd-vaas-publish-videos`
   ...
   Video upload runs a **unified py runtime** (patchright): 5 Playwright platforms go through a
   vendored copy of `social-auto-upload` under `scripts/upstream/` via the thin adapter
   `scripts/platforms/sau_adapter.py`; `bilibili.py` stays local (upstream uses the `biliup` binary,
   not Playwright). `sync-upstream.sh` re-syncs the vendor from the upstream repo.
...
demand / 文案
  ...
  -> /fd-vaas-publish-videos       (.mp4 -> 6 video platforms;  unified py runtime: vendored upstream + bilibili.py)
...
via its built-in uploaders under `scripts/platforms/` (thin adapter `sau_adapter.py` → vendored
  upstream; `bilibili.py` local).*
...
- **social-auto-upload is vendored** under `.agents/skills/fd-vaas-publish-videos/scripts/upstream/`
  (uploader/+utils/+conf.example.py). The thin adapter `sau_adapter.py` translates CLI → upstream
  `<Platform>Video(...).main()`. `sync-upstream.sh` re-syncs the vendor from the upstream repo.
  Local overrides live in `conf.py` + `sau_adapter.py` — do **not** hand-edit
  `scripts/upstream/uploader|utils` (canonical; `--delete` rsync will clobber). Cookies/logs under
  `scripts/upstream/` are runtime state, gitignored, never committed.
...
| `fd-vaas-publish-videos` | one video -> 6 video platforms (unified py runtime: vendored upstream + `bilibili.py`) | repo skill | `/fd-vaas-publish-videos` |
...
One video = one `publish.mjs` call -> per-platform preferences + per-platform upload. All 6 platforms
use a **unified py runtime** (patchright): 5 Playwright platforms route through the thin adapter
`sau_adapter.py` → vendored upstream (`social-auto-upload`); `bilibili` stays local (`bilibili.py`,
since upstream uses the `biliup` binary, not Playwright):

| Platform | Upload path | Key technical challenge |
|---|---|---|
| `douyin` | `sau_adapter` → upstream | standard DOM |
| `xiaohongshu` | `sau_adapter` → upstream ✓已实发 | title ≤ 20 字; tag ≤ 10 |
| `bilibili` | `bilibili.py` (上游用 biliup) | **micro-app shadow DOM**; needs `--tid`; `--cover` not `--thumb` |
| `kuaishou` | `sau_adapter` → upstream | **React Joyride overlay** + off-screen publish btn; tag ≤ **4** |
| `weixin` (视频号) | `sau_adapter` → upstream | **Wujie shadow DOM** |
| `youtube` | `sau_adapter` → upstream（需代理） | **Polymer dialog** + 4-step flow; `--visibility`; "not for kids" mandatory |

Login state: `scripts/upstream/cookies/<platform>_uploader/account.json` (patchright `storage_state`,
gitignored). Per-platform deep dives in `references/<platform>.md` (runtime-agnostic facts: shadow DOM
structure, char limits); quirks in `references/platform-quirks.md`.

**Verified:** xiaohongshu end-to-end (vendored upstream hits `wait_for_url("**/publish/success?**")`).
Other Playwright platforms + `bilibili` shadow-DOM still need real-machine verification on a logged-in
session — run `--dry-run` first, confirm the flow reaches the upload page before trusting a real publish.
...
# single-platform ad-hoc upload (debugging, goes through vendored upstream)
python3 $SKILL_P/platforms/sau_adapter.py --platform douyin --file <mp4> --title "…" --desc "…" --tags a,b
```
...
- **Login model**: **video** (`fd-vaas-publish-videos`) — patchright `storage_state` cookies at
  `scripts/upstream/cookies/<platform>_uploader/account.json` (scan-login via
  `sau_adapter.py --platform <p> --login`; `--migrate-profile` from old `.profiles/`). **docs**
  (`fd-vaas-publish-docs`) — ego-browser inherits the user's Chrome login state (login once in a real
  browser, reuse thereafter; no cookie files). Each platform's session is independent - multi-platform
  concurrency OK, same-platform must be serial.
- **Close the ego task window after publish** (docs skill only) - do not leave it for the user to close.
  The video skill's patchright runtime self-closes (no ego window). For douyin's manual-publish path,
  wait for the user to confirm "发布完成" before running the `completeTaskSpace` cleanup.
...
- **Creation -> distribution handoff**: the `fd-vaas-*` mainline wires this end-to-end
  ...
  `python3 .agents/skills/fd-vaas-publish-videos/scripts/platforms/sau_adapter.py --platform <p> --file <path> …`
